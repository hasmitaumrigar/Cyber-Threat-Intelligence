# app.py — Upgraded Threat Intelligence Dashboard
...
if not high_risk_ips.empty:
    ...
    st.table(high_risk_ips[["IP", "Country", "ISP", "Abuse Score", "Reports"]])
...
if st.button("Check Threat") and ip:
    ...
    vt_data = check_virustotal(ip, VT_KEY)
    otx_data = check_otx(ip, OTX_KEY)

    # Unified score and risk
    ...
    # Append to CSV
    df = pd.DataFrame([result], columns=["IP","Country","ISP","Abuse Score","Reports","Risk"])
...
st.write("High Risk IPs:", len(history[history["Risk"]=="High Risk 🔴"]))
st.write("Medium Risk IPs:", len(history[history["Risk"]=="Medium Risk 🟠"]))
st.write("Low Risk IPs:", len(history[history["Risk"]=="Low Risk 🟢"]))
st.write("Top Countries:", history["Country"].value_counts().head(5))
